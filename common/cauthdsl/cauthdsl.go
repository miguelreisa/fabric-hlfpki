...
func compile(policy *cb.SignaturePolicy, identities []*mb.MSPPrincipal, deserializer msp.IdentityDeserializer, sigMethod []byte) (func([]*cb.SignedData, []bool) bool, error) {
	if policy == nil {
	...
	case *cb.SignaturePolicy_NOutOf_:
		...
		for i, policy := range t.NOutOf.Rules {
			compiledPolicy, err := compile(policy, identities, deserializer, sigMethod)
			if err != nil {
				return nil, err
			}
	...
	case *cb.SignaturePolicy_SignedBy:
		...
		return func(signedData []*cb.SignedData, used []bool) bool {
			...
			for i, sd := range signedData {
				...
				// FGODINHO
				if sigMethod == nil || string(sigMethod) == "multisig" {
					// if signature method is null (used by the system of hlf) or multisig,
					// go right into using default multisig identity sig verification
